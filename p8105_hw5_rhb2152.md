...
The graph below shows the power of the test compared to the true value
of $\mu$. The null hypothesis is fixed as $\mu=0$, and therefore, as the
true value of $\mu$ increases from 1, the power quickly increases and
then starts to plateau until it reaches essentially 100% at $\mu=6$.
...
As the first plot below shows, when the average estimates of $\mu$ are
plotted against the true value, they essentially equal each other, since
the points fall along the line $y=x$. This makes sense because many
samples are taken from the distribution with the true $\mu$.

However, in the second plot, when we only consider samples that reject
the null hypothesis, the average estimate needs to be greater than the
true value at values close to the null. This provides sufficient
evidence to reject the null. That is, when the true value is $\mu=1$ for
example, samples generated will likely overlap with zero, making it
harder to reject the null. However, at values farther from the null, the
null is typically rejected anyways, so the average estimate approximates
the true value (reflects the first plot since many samples are not
filtered out). The one difference is that when the true value is zero,
there aren’t many samples that reject the null, so the average of the
samples that *do* reject would still have to be higher than zero, but
not that much since the distribution which these samples came from is
...
The table below shows the number of solved and unsolved homicides across
all cities.
...
The table below shows the estimate and 95% confidence interval for the
...
The graph below shows the estimate for the proportion of unsolved
homicides in each city along with their 95% confidence interval. The
graph is sorted from greatest proportion to least.
...
![](p8105_hw5_rhb2152_files/figure-gfm/unnamed-chunk-7-1.png)<!-- -->
